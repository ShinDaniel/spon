...
#if defined(NDEBUG)
#error "SPON cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < 86400 && nHeight > 23) {
        nSubsidy = 25000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 43200) {
        ...
        int64_t mNodeCoins = mnodeman.size() * 10000000 * COIN;

        //if a mn count is inserted into the function we are looking for a specific result for a masternode count
        if(nMasternodeCount)
            mNodeCoins = nMasternodeCount * 10000000 * COIN;
